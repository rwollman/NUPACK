...
int main( int argc, char *argv[] ) {
  ...
  int thepairs[MAXSEQLENGTH];
    ...
    if(NupackShowHelp) {
      printf("Usage: mpirun [MPIOPTIONS] pairs_mpi [OPTIONS] PREFIX\n");
      printf("Calculate the pair probabilities of the input sequence.\n");
      printf("Example: mpirun -np 8 utils_mpi -multi -T 25 -material dna example\n");
    ...
    printInputs( argc, argv, seqChar, vs, NULL, parens, ppairsFile);
    ...
    if(parens[0] != '\0') {
      getStructureFromParens(parens, thepairs, length);
    }

    DBL_TYPE structure_free_energy = naEnergyPairsOrParensFullWithSym(thepairs,NULL,seqNum,
            DNARNACOUNT,DANGLETYPE,TEMP_K - ZERO_C_IN_KELVIN, vs, SODIUM_CONC, MAGNESIUM_CONC,
    ...
    int target_pair;
    DBL_TYPE ensemble_defect = 0;
    for(i = 0 ; i < length ; i++) {
      target_pair = thepairs[i];
      if(target_pair == -1) {
        target_pair = length;
      }
      ensemble_defect += 1 - pairPr[i*(length+1) + target_pair];
    }

    // Print the free energy to the output file
    if(!NUPACK_VALIDATE) {
      fprintf(F_permPr,"%s Structure free energy: %.8Le kcal/mol\n",
            COMMENT_STRING,(long double)structure_free_energy);
      fprintf(F_permPr,"%s Ensemble free energy: %.8Le kcal/mol\n",
            COMMENT_STRING,(long double)-kB*TEMP_K*logl(pf));
      fprintf(F_permPr,"%s Partition function: %.8Le\n",
            COMMENT_STRING,pf);
      fprintf(F_permPr,"%s Probability: %.8Le\n",
            COMMENT_STRING,(long double) EXP_FUNC(-structure_free_energy/(kB*TEMP_K))/pf);
      fprintf(F_permPr,"%s Ensemble defect: %.8Le\n",
            COMMENT_STRING,(long double) ensemble_defect);
      fprintf(F_permPr,"%s Normalized ensemble defect: %.8Le\n",
            COMMENT_STRING,(long double) ensemble_defect / length);
    } else {
      fprintf(F_permPr,"%s Structure free energy: %.14Le kcal/mol\n",
            COMMENT_STRING,(long double) structure_free_energy);
      fprintf(F_permPr,"%s Ensemble free energy: %.14Le kcal/mol\n",
            COMMENT_STRING,(long double) -kB*TEMP_K*logl(pf));
      fprintf(F_permPr,"%s Partition function: %.14Le\n",
            COMMENT_STRING,(long double) pf);
      fprintf(F_permPr,"%s Probability: %.14Le\n",
            COMMENT_STRING,(long double) EXP_FUNC(-structure_free_energy/(kB*TEMP_K))/pf);
      fprintf(F_permPr,"%s Ensemble defect: %.14Le\n",
            COMMENT_STRING,(long double) ensemble_defect);
      fprintf(F_permPr,"%s Normalized ensemble defect: %.14Le\n",
            COMMENT_STRING,(long double) ensemble_defect / length);
    }
